...
export const zeebeIncidentsPerMessageGauge = new client.Gauge({
    ...
    async collect() {
      ...
      if (ldbResults?.incidentCountPerMessage) {
        // convert to Map
        const incidentCountPerMessageMap = new Map<string, number>( Object.entries(ldbResults.incidentCountPerMessage) )
        // set the measure
        tallyHistogram(incidentCountPerMessageMap).forEach((count: number, message: string) => {
          this.set(
